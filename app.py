...
if report_type == 'Single Stock Outlook':
    ...
    if symbol:
        with st.spinner(f'Generating report for {symbol}...'):
            response = query_engine.query(f"Write a report on the outlook for {symbol} stock from the years 2023-2027. Be sure to include potential risks and benefits.")
...
if report_type == 'Competitor Analysis':
    ...
    if symbol1 and symbol2:
        with st.spinner(f'Generating report for {symbol1} vs. {symbol2}...'):
            response = query_engine.query(f"Write a report on the competition between {symbol1} stock and {symbol2} stock. Be sure include potential risks and benefits.")
            st.write(response.get_response().response)
